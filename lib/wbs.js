...
export function normalizarPartidaWbs(partida, codigo) {
  ...
  // El sufijo embebido en `partida` manda sobre la columna `codigo`: la
  // migración 0017 le asignó un entero secuencial de respaldo a toda fila
  // con parent_id null (pensado para categorías raíz reales), pero en este
  // catálogo importado de Excel TODAS las filas tienen parent_id null —
  // incluidas las hojas de gasto, que ya traían su código real de 3 niveles
  // en el sufijo. Si se le diera prioridad a `codigo` se mostraría ese
  // entero de posición en vez del código WBS real.
  if (match) {
    return { codigo: match[1], partida: texto.replace(SUFIJO_CODIGO_RE, "").trim() };
  }
  if (codigo) {
    return { codigo, partida: texto };
  }
 ...
 *      "1.1", que a su vez cuelga de "1") **y** existe al menos una fila real
 *      para algún nivel intermedio de esos códigos. El catálogo real
 *      importado de Excel no cumple esto último: cada hoja ya trae su código
 *      completo de 3 niveles embebido en `partida` (`normalizarPartidaWbs`,
 *      desde el fix de "Corrige código mostrado en selector de Partida WBS")
 *      pero nunca hay una fila propia para el nivel 1 o 2 de ese código — sin
 *      ningún padre real que encontrar, este modo cae al punto 3 en vez de
 *      dejar cada hoja como su propia "raíz" suelta (bug corregido junto con
 *      la sección R2/XML de CLAUDE.md, ver esa sección de "0035").
 *   3. Catálogo plano (caso real de las 500+ partidas importadas desde
 *      Excel): ninguna fila trae `parent_id` ni jerarquía real de `codigo`,
 *      solo el texto repetido de `categoria` y `partida` en cada renglón (sin
 *      una columna de subpartida propia). Se arman dos niveles virtuales — no
 ...
 */
/**
 * Agrupa virtualmente un catálogo plano (sin `parent_id` ni jerarquía real de
 * `codigo`) en dos niveles sintéticos — categoría y partida madre, ninguno
 * de los dos es una fila real de `wbs_catalog` — dejando las filas reales
 * como hojas de Nivel 3. Extraído de `construirArbol` para poder reusarse
 * como fallback cuando la rama de `codigo` de puntos no encuentra ningún
 * padre real (ver nota en el punto 2 del docstring de `construirArbol`).
 */
function agruparPorCategoriaYPartida(porId) {
  const raices = [];
...
export function construirArbol(filas, orden) {
  ...
  if (tieneParentIdExplicito) {
    ...
    let huboJerarquiaReal = false;
    porId.forEach((nodo) => {
      const padre = porCodigo.get(codigoPadreInferido(nodo.codigo));
      if (padre) {
        padre.hijos.push(nodo);
        huboJerarquiaReal = true;
      } else {
        raices.push(nodo);
      }
    });
    // Catálogo importado de Excel donde cada hoja ya trae su código completo
    // de 3 niveles embebido en `partida` (ej. "(1.2.1)", ver
    // normalizarPartidaWbs) pero no existen filas reales para los niveles 1
    // y 2 de ese código (solo la hoja) — `codigoPadreInferido` nunca
    // encuentra un padre real para nadie y todo termina como 540 "raíces"
    // sueltas en vez de un árbol. Sin jerarquía real que aprovechar, cae al
    // agrupado virtual por categoría/partida de abajo.
    if (!huboJerarquiaReal) {
      raices = agruparPorCategoriaYPartida(porId);
    }
  } else {
    raices = agruparPorCategoriaYPartida(porId);
  }
