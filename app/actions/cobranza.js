...
export async function getDesglosePagosCliente(clienteId) {
  ...
  const { data, error } = await supabase
    .from("contratos_venta")
    .select(
      "id, monto_total_venta, fecha_contrato, esquema_venta, contrato_firmado, unidades(codigo_unidad, tipo_uso, superficie_m2, precio_m2, monto_lista, esquema_unidad), proyectos(id, codigo, nombre, logo_proyecto_url, logo_compacto_url, color_primario, color_secundario), planes_pago_cobranza(id, tipo_pago, monto_programado, monto_pagado, fecha_programada, fecha_pago, estatus, fase_plan, notas)"
    )
