...
export default async function ConfiguracionPlantillasPage() {
  const [plantillas, proyectos] = await Promise.all([getConfiguracionPlantillas(), getProyectosBranding()]);
  ...
  return (
    <div className="flex flex-col flex-1 items-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-7xl mx-auto flex-col gap-6 py-16 px-4">
        ...
        <PanelConfiguracionPlantillas plantillas={plantillas} proyectos={proyectos} />
      </main>
    </div>
  );
}
